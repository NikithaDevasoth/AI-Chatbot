  ...
  return (
    <div
      className={`flex flex-col h-screen min-w-72 p-5 
        ...
        border-r border-r-[#80609F]/30 backdrop-blur-3xl 
    ...
    >
      ...
      <input
        ...
        className="mt-5 px-3 py-2 text-sm rounded-md border 
          border-gray-300 dark:border-[#80609F]/30 bg-transparent 
        ...
        {chats
          ...
          .map((chat) => (
            <div onClick={()=>{navigate('/');setSelectedChat(chat);setIsMenuOpen(false)}}
              key={chat._id}
              className="p-2 px-4 dark:bg-[#57137C]/10 border 
...
<div className="flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 
             rounded-md ">
              <div className="flext items-center gap-2 text-sm">
                <img
                src={assets.theme_icon} className="w-4 not-dark:invert" alt="Theme Icon"/>
                <p>Dark Mode</p>

              </div>
              <label className="relative inline-flex cursor-pointer">
                <input onClick={()=>setTheme(theme==="dark"?"light":"dark")} type="checkbox" className="sr-only
                peer" checked={theme==="dark"}/>
...
cursor-pointer md:hidden not-dark:inver" alt=""/>
 </div>
  );
};
